...
const PostWidget = ({categories, slug}) => {
  ...
  return (
    <div className="bg-white bg-opacity-70 shadow-lg rounded-lg p-8 pb-12 mb-8">
      <h3 className="text-xl mb-8 font-semibold border-b border-pink-600 pb-4">{slug ? 'Related Posts' : 'Recent Posts'}</h3>
      {relatedPosts.map((post, index) => (
        <div key={index} className="flex items-center w-full mb-4">
          ...
          <div className="flex-grow ml-4">
            ...
            <Link href={`/post/${post.slug}`} key={index}><span  className=" cursor-pointer text-md transition duration-300 hover:text-pink-600">{post.title}</span></Link>
          </div>
        </div>
      ))}
    </div>
